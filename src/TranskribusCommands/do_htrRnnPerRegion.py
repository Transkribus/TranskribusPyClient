...
description = """Apply an HTR RNN model for a given table column with a specific dictionary.

""" + _Trnskrbs_description

usage = """%s <modelID>  <dictname> <colId> [--trp] [--docid] [--colnum] [--dict] [--tempdict]
"""%sys.argv[0]

class DoHtrRnnPerColumn(TranskribusClient):
    """
    ...
    def buildDescription(self,colId,docpage,colnum,trp=None):
        """
            '{"docId":17442,"pageList":{"pages":[{"pageId":400008,"tsId":1243509,"regionIds":[]}]}}'
        """
#         return  17442,'{"docId":17442,"pageList":{"pages":[{"pageId":400008,"tsId":1243509,"regionIds":[]}]}}'.encode('utf-8')
        jsonDesc = {}
...
#         pList= trpObj.getTranscriptList()
        ...
        for page in trpObj.getPageList():
            ## need to upload the page!!!!
            regionsIDs=[]
            docId = page['docId']
            jsonDesc["docId"]=page['docId']
            jsonDesc["pageList"]['pages'].append({"pageId":page['pageId'],"tsId":page['tsList']['transcripts'][0]['tsId'],"regionIds":regionsIDs})        
...
if __name__ == '__main__':
    ...
    __Trnskrbs_basic_options(parser, DoHtrRnnPerColumn.sDefaultServerUrl)
        
    parser.add_option("-r", "--region"  , dest='region', action="store", type="string", default=DoHtrRnnPerColumn.sDefaultServerUrl, help="apply HTR at region level")
    ...
    parser.add_option("--colnum"  , dest='colnum'   , action="store", type="string", default=None, help="column to be htr'd")
...
# ---   
    ...
    # --- 
    doer = DoHtrRnnPerColumn(options.server, proxies, loggingLevel=logging.WARN)
...
#     except Exception as e:      sPages = None
    ...
    if options.trp_doc:
        ...
        docId,sPageDesc = doer.buildDescription(colId,options.docid,options.colnum,trpdoc)
    else:
        docId,sPageDesc = doer.buildDescription(colId,options.docid,options.colnum)
